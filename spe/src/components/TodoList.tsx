...
function AnnualGoalItem({
  ...
  return (
    <div className="border border-gray-700 rounded-lg p-2.5">
        ...
        <div className="mt-2 ml-2 border-l border-gray-700 pl-2 space-y-2">
          ...
          {monthlyGoals.map((monthlyGoal) => (
            <MonthlyGoalItem
              key={monthlyGoal.id}
              goal={monthlyGoal}
              isExpanded={expandedGoals[monthlyGoal.id]}
              onToggle={() => setExpandedGoals(prev => ({ ...prev, [monthlyGoal.id]: !prev[monthlyGoal.id] }))}
              allGoals={allGoals}
...
function MonthlyGoalItem({
  ...
  const weeklyGoals = allGoals.filter(g => g.period_type === "weekly" && g.parent_id === goal.id);
    ...
    <div className="bg-gray-800/50 rounded p-2">
      <button onClick={onToggle} className="w-full text-left">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-300 flex-1">{goal.title}</span>
          <span className="text-xs text-gray-600">{goal.current_value ?? 0}/{goal.target_value ?? "?"}{goal.unit ?? ""}</span>
        </div>
      </button>
      <div className="mt-1 ml-4">
        ...
        <div className="text-right text-xs text-gray-700 mt-0.5">{progress}%</div>
      </div>
      {isExpanded && weeklyGoals.length > 0 && (
        <div className="mt-2 ml-2 border-l border-gray-700 pl-2 space-y-1">
          ...
          {weeklyGoals.map((weeklyGoal) => {
            ...
            const weekBarColor = weekProgress >= 80 ? "bg-green-500" : weekProgress >= 40 ? "bg-yellow-500" : "bg-red-500";
            return (
              <div key={weeklyGoal.id} className="bg-gray-900/50 rounded p-1.5">
                ...
                <div className="w-full h-1 bg-gray-700 rounded-full overflow-hidden">
                  <div className={`h-full transition-all duration-300 ${weekBarColor}`} style={{ width: `${weekProgress}%` }} />
                </div>
      ...
      <div className="overflow-y-auto" style={{ maxHeight: "calc(100vh - 180px)" }}>
        ...
        {currentTab === "goals" && (
          <div className="flex flex-col h-full">
            <div className="px-3 py-2 border-b border-gray-800">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">目標進捗</span>
                        ...
                        <div className="space-y-2 ml-2">
                          {annualGoals.map((goal) => (
                            <AnnualGoalItem
                              ...
                              onToggle={() => setExpandedGoals(prev => ({ ...prev, [goal.id]: !prev[goal.id] }))}
                              allGoals={allGoals}
                              expandedGoals={expandedGoals}
                              setExpandedGoals={setExpandedGoals}
                            />
